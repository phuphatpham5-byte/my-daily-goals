...
function restoreLayout() { 
    ...
    // Khôi phục kích thước lưu trước đó
    Object.keys(appData.moduleWidths).forEach(id => {
        const el = document.getElementById(id);
        if (el) el.style.flex = `0 0 ${appData.moduleWidths[id]}`;
    });
    Object.keys(appData.moduleHeights).forEach(id => {
        const el = document.getElementById(id);
        if (el) el.style.minHeight = appData.moduleHeights[id];
    });
}

/* --- TÍNH NĂNG MỚI: ĐIỀU CHỈNH KÍCH THƯỚC ĐA CHIỀU (NGANG, DỌC, GÓC) --- */
function initResizing() {
    let currentModule = null;
    let startX, startY, startWidth, startHeight, currentMode = '';

    // Hàm chung bắt đầu kéo
    function startResize(e, mode) {
        e.preventDefault();
        currentMode = mode;
        currentModule = e.target.parentElement;
        startX = e.clientX;
        startY = e.clientY;
        startWidth = currentModule.offsetWidth;
        startHeight = currentModule.offsetHeight;

        currentModule.setAttribute('draggable', 'false'); // Khóa di chuyển khi đang kéo
        currentModule.classList.add('resizing');
        document.addEventListener('mousemove', handleMouseMove);
        document.addEventListener('mouseup', handleMouseUp);
    }

    // Gắn sự kiện cho các thanh kéo
...
/* --- GOALS & HABITS (GIỮ NGUYÊN CODE CŨ) --- */
function addGoal() {
    ...
    if (endDate && endDate >= currentDate) {
        appData.habits.push({ id: 'h_' + Date.now(), title: title, startDate: currentDate, endDate: endDate, progressMap: {} });
...
function renderHabits() {
    ...
    hr.innerHTML += `<th>Progress</th><th>...</th>`;
    thead.appendChild(hr); container.appendChild(thead);
...
/* --- SHEETS & WHEEL (GIỮ NGUYÊN CODE CŨ) --- */
...
/* --- FLASHCARDS & POMODORO (GIỮ NGUYÊN CODE CŨ) --- */
